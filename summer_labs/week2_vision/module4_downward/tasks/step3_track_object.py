...
# -- Module-level state -----------------------------------------------------
_hold = 0.0 ## dynamic holding time 
_done = False
...
def update(drone):
    ...
    brightest = neo_lab.largest_bright_contour(image,V_MIN, MIN_AREA)

    if (brightest.all() == None):
        drone.flight.stop()
        _hold = 0.0
        return False
    target_x, target_y = uav_utils.get_contour_center(brightest)
    err_x = target_x - ROW_CENTER
    err_y = target_y - COL_CENTER
    roll = uav_utils.clamp(err_x*MAX_TILT, -MAX_TILT, MAX_TILT)
    pitch = uav_utils.clamp(err_y*MAX_TILT, -MAX_TILT, MAX_TILT)

    if(abs(err_x)<=CENTER_TOL):
        if(abs(err_y)<=CENTER_TOL):
            print("x and y within error")
            drone.flight.stop()
            drone.flight.land()
            _done = True
        else:
            print("x within error, y not within error")
            drone.flight.send_pcmd(pitch,0,0,0)
    else:
        if(abs(err_y)<=CENTER_TOL):
            print("x not within error, y within error")
            drone.flight.send_pcmd(0,roll,0,0)
        else:
            print("x and y not within error")
