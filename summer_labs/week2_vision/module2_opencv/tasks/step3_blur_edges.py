...
def update(drone):
    ...
    blurred_img = cv2.blur(grey_img, (KERNEL_SIZE, KERNEL_SIZE))
    filtered_x = cv2.Sobel(blurred_img,cv2.CV_64F, 1, 0, ksize = 3)
    # print(f"x: {filtered_x}")
    filtered_y = cv2.Sobel(blurred_img,cv2.CV_64F, 0, 1, ksize = 3)
    # print(f"y: {filtered_y}")
    magnitude = np.sqrt(filtered_x ** 2 + filtered_y ** 2)
    # magnitude = cv2.magnitude(filtered_x, filtered_y)
    
    _timer += drone.get_delta_time()
    if (_timer >= HOVER_TIME):
        print(f"Magnitude: {magnitude.mean():.2f}")
        _done = True 
